...
docker run --rm --gpus all --ipc=host --shm-size=8g \
  ...
  "$image_name" \
  bash -lc "source /opt/ros/humble/setup.bash && python /workspace/project/scripts/patch_openpcdet_optional_datasets.py /workspace/OpenPCDet && python /workspace/project/scripts/patch_openpcdet_numpy_compat.py /workspace/OpenPCDet && python /workspace/project/scripts/create_nuscenes_temporal_config.py /workspace/OpenPCDet --max-sweeps 3 && export PYTHONPATH=/workspace/OpenPCDet && cd /workspace/project/ros2_ws && colcon build --symlink-install && source install/setup.bash && ros2 launch robot_3d_detection replay_temporal_detect.launch.py data_root:=$dataset_dir manifest:=/workspace/project/artifacts/e003_bev/manifest.json cfg_file:=/workspace/OpenPCDet/tools/cfgs/nuscenes_models/pointpillar_nuscenes_mini_3sweeps.yaml ckpt:=$container_checkpoint"
