...
docker run --rm --gpus all --ipc=host \
  ...
  "$image_name" \
  bash -lc "python /workspace/project/scripts/patch_openpcdet_optional_datasets.py /workspace/OpenPCDet && python /workspace/project/scripts/create_nuscenes_temporal_config.py /workspace/OpenPCDet --max-sweeps $max_sweeps && export PYTHONPATH=/workspace/OpenPCDet && cd /workspace/OpenPCDet/tools && python -m pcdet.datasets.nuscenes.nuscenes_dataset --func create_nuscenes_infos --cfg_file cfgs/dataset_configs/nuscenes_mini_${max_sweeps}sweeps.yaml --version v1.0-mini"
