...
def generate_launch_description():
    ...
    detector = Node(
        ...
        name="pointpillar_temporal_detector",
        parameters=[{
            "cfg_file": cfg_file,
            "ckpt": ckpt,
    ...
    shutdown_after_replay = RegisterEventHandler(
        OnProcessExit(target_action=replay, on_exit=[EmitEvent(event=Shutdown(reason="temporal nuScenes replay complete"))])
    )
